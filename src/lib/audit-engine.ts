...
export function detectOverlaps(subscriptions: AuditInput[]) {
  const recommendations = [];

  const hasChatGPT = subscriptions.some((sub) => sub.tool === "ChatGPT");
  const hasClaude = subscriptions.some((sub) => sub.tool === "Claude");
  const hasCursor = subscriptions.some((sub) => sub.tool === "Cursor");
  const hasCopilot = subscriptions.some((sub) => sub.tool === "Copilot");

  if (hasChatGPT && hasClaude) {
    recommendations.push({
      title: "Potential overlap between ChatGPT and Claude",
      savings: 20,
      reason:
  ...
  if (hasCursor && hasCopilot) {
    recommendations.push({
      title: "Potential overlap between Cursor and Copilot",
      savings: 19,
      reason:

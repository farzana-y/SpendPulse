...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    const overlapSavings = (result.overlaps ?? []).reduce((sum, o) => sum + o.savings, 0);
  ...
  const totalOverlapSavings = (result.overlaps ?? []).reduce((sum, o) => sum + o.savings, 0);
  const auditSavings = result.audits.reduce((sum, a) => sum + a.savings, 0);
    ...
    <main className="min-h-screen bg-zinc-950 text-white">
      ...
      <div className="mx-auto max-w-3xl px-4 py-12 space-y-10">
        ...
        <div>
          ...
          <h1 className="text-4xl font-bold mt-4 tracking-tight">Your AI Spend Audit</h1>
        ...
        {isOptimal ? (
          <div className="rounded-2xl border border-emerald-800 bg-emerald-950/40 p-8 text-center">
            ...
            <p className="text-zinc-400 mt-2">No major optimizations detected for your current stack.</p>
          </div>
        ) : (
          <div className="rounded-2xl border border-emerald-700 bg-emerald-950/50 p-8">
            ...
            <div className="flex flex-col sm:flex-row gap-8">
              <div>
                ...
                <p className="text-5xl font-bold text-emerald-400 mt-1">${totalMonthlySavings}</p>
              </div>
              <div className="sm:border-l border-zinc-700 sm:pl-8">
                <p className="text-zinc-400 text-sm">Annual</p>
                <p className="text-5xl font-bold text-white mt-1">${totalAnnualSavings}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6">
          <div className="flex items-center justify-between mb-3">
            ...
            {usedFallback && (
              <span className="text-xs text-amber-500">AI unavailable — showing template</span>
            )}
          </div>
          {summaryLoading ? (
        ...
        {isHighSavings && (
          <div className="rounded-2xl border border-amber-700 bg-amber-950/40 p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div className="flex-1">
              <h3 className="font-bold text-amber-400 text-lg">You could save even more with Credex</h3>
              <p className="text-zinc-400 text-sm mt-1">
                Credex sells discounted AI credits for Cursor, Claude, ChatGPT Enterprise and more —
                sourced from companies that overforecast. Book a free consultation to see what&apos;s available for your stack.
              </p>
            </div>
            <a
              ...
              {result.subscriptions.map((sub, index) => (
                <div key={index} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5 space-y-2 text-sm">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="bg-zinc-800 text-zinc-300 px-2 py-0.5 rounded-md text-xs">{sub.plan}</span>
                  </div>
